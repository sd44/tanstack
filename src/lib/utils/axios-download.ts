...
export function downloadFileWithAxios(response: BlobResponse) {
  try {
    // 1. 从响应头获取文件名
    ...
    if (contentDisposition) {
      ...
      const filenameMatch = contentDisposition.match(FilePattern);
      if (filenameMatch?.[1]) {
        try {
    ...
    console.error('下载文件时发生错误:', _error);
  }
}
