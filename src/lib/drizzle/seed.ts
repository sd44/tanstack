...
import { industryCodeList } from '~/lib/db/CRUD/entry-datas';
import { norm_industry, user, visits } from '~/lib/db/schema';
import { enterprises, type enterprisesInserT } from '~/lib/db/schema/enterprises.schema';
  ...
  try {
    ...
    for (let i = 0; i < 1000; i++) {
      datas.push({
        ...
        registeredCapital: faker.number.float({
          min: 0,
          max: 3000,
          fractionDigits: 1,
        }),
    ...
    await db.insert(enterprises).values(datas).onConflictDoNothing();
