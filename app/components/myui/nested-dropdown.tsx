    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        ...
        <div className={cn('relative w-full cursor-pointer', inputClassName)}>
          <Input
            value={selectedLabel ?? ''} // Display selected label or empty string
            placeholder={labelName} // Use labelName as placeholder
            readOnly // Make it non-editable
            className="pr-12" // Add padding to prevent text overlap with icons
          />
          ...
          {selectedValue && (
            <Button
              variant="ghost"
              size="icon"
              className="text-muted-foreground hover:text-foreground absolute top-1/2 right-7 z-10 h-5 w-5 -translate-y-1/2 rounded-full" //z-10确保按钮在最上层
          ...
          {/* Dropdown Indicator */}
          <ChevronDownIcon
            className="absolute top-1/2 right-2.5 h-4 w-4 -translate-y-1/2 opacity-50" // Position icon
            aria-hidden="true"
          />
        </div>
      </DropdownMenuTrigger>
