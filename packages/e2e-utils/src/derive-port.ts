 ...
 */
export function derivePort(input: string, min: number = 5600, max: number = 65_535): number {
  // Hash the input using SHA-256
  const hash = crypto.createHash('sha256').update(input).digest('hex');

  // Convert hash to an integer
  const hashInt = Number.parseInt(hash.slice(0, 8), 16); // Use the first 8 characters
